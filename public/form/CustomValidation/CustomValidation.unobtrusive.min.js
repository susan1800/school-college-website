...
(function () {
    ...
    jQuery.validator.addMethod("dateformatvalid", function (value, element, param) {
    ...
    jQuery.validator.addMethod("filesize", function (value, element, param) {
        ...
        var _validFileExtensions = param.validfiletypes.split(",");
        var sizeThreshold = param.maxmb;

        const file = $(element);

        const fsize = file[0].files[0].size;
        ...
        var sFileName = file[0].files[0].name.split('.').pop();;
    ...
    jQuery.validator.addMethod("validaitygraterthanisssuedate", function (b, d, e) {
        var c = foolproof.getId(d, e.dependentproperty),
            a = document.getElementById(c).value;
        if (a.length > 0 && b.length > 0) {

            var issueDate = Date.parse(a);
            var expiryDate = Date.parse(b);
    ...
    jQuery.validator.addMethod("minagevalid", function (value, element, param) {
    ...
    b.adapters.add("requiredif", ["dependentproperty", "dependentvalue", "operator", "pattern"], function (b) {
        var c = { dependentproperty: b.params.dependentproperty, dependentvalue: b.params.dependentvalue, operator: b.params.operator, pattern: b.params.pattern };
        a(b, "requiredif", c);
    });
    b.adapters.add("is", ["dependentproperty", "operator", "passonnull"], function (b) {
        a(b, "is", { dependentproperty: b.params.dependentproperty, operator: b.params.operator, passonnull: b.params.passonnull });
    });
    b.adapters.add("requiredifempty", ["dependentproperty"], function (b) {
        a(b, "requiredifempty", { dependentproperty: b.params.dependentproperty });
    });
    b.adapters.add("requiredifnotempty", ["dependentproperty"], function (b) {
        a(b, "requiredifnotempty", { dependentproperty: b.params.dependentproperty });
    });

    b.adapters.add("dateformatvalid", function (b) {
        a(b, "dateformatvalid", {});
    });

    b.adapters.add("filesize", ["maxmb", "validfiletypes"], function (b) {
        a(b, "filesize", { maxmb: b.params.maxmb, validfiletypes: b.params.validfiletypes });
    });

    b.adapters.add("validaitygraterthanisssuedate", ["dependentproperty"], function (b) {
        a(b, "validaitygraterthanisssuedate", { dependentproperty: b.params.dependentproperty });
    });

    b.adapters.add("minagevalid", function (b) {
        a(b, "minagevalid", {});
    });
