                        ...
                        @foreach($blogs as $blog)
                            @if ($blog->id != 0)
                                <tr>
                                    ...
                                    <td>{!! $blog->about_author !!}</td>


                                    <td class="text-center">
                                        <div class="btn-group" role="group" aria-label="Second group">
                                            @if($blog->status == 1)
                                            <a href="{{ route('admin.blogs.disable', $blog->id) }}" class="btn btn-sm btn-primary" style="margin:3px;"> <i class="fa fa-eye" aria-hidden="true"></i> </a>
                                            @else
                                            <a href="{{ route('admin.blogs.disable', $blog->id) }}" class="btn btn-sm btn-danger" style="margin:3px;"> <i class="fa fa-eye-slash" aria-hidden="true"></i> </a>
                                            @endif
                                            <a href="{{ route('admin.blogs.edit', $blog->id) }}" class="btn btn-sm btn-primary"><i class="fa fa-edit"></i></a>
                                            <a onclick="deletefunction('<?= $blog->id ?>')" class="btn btn-sm btn-danger"><i class="fa fa-trash"></i></a>
                                        </div>
                                    </td>
                                </tr>
                            @endif
...
Swal.fire({
  ...
  if (result.isConfirmed) {
    $.ajax({url: "https://uttarpustika.com/admin/blogs/"+id+"/delete", success: function(result){
